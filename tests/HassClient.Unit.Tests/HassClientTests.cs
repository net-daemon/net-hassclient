    ...
    public class HassClientTests
    {
        ...
        public async void TestCallServiceGeneralException()
        {
            var webSocketMock = Mock.Of<IClientWebSocket>(ws =>
                ws.SendAsync(null, WebSocketMessageType.Text, true, CancellationToken.None) == Task.FromException(new Exception("Some exception")));
        ...
        //        It.IsAny<WebSocketMessageType>(), It.IsAny<CancellationToken>()))
        //    //var messages = new Queue<IPageModel>();
        //    //var webSocketMock = Mock.Of<IClientWebSocket>(ws =>
        //    //    ws.ConnectAsync(new Uri("ws://localhost:8123"), CancellationToken.None) == Task.Delay(2) &&
        //    //    ws.CloseAsync(WebSocketCloseStatus.Empty, "", CancellationToken.None) == Task.Delay(50) &&
        //    //    ws.SendAsync(ArraySegment<byte>.Empty, WebSocketMessageType.Text, true, CancellationToken.None) == Task.Delay(2) &&
        //    //    ws.ReceiveAsync(It.IsAny<Memory<byte>>(), It.IsAny<CancellationToken>) == (buffer, cancel) => 
        //    //        new ValueWebSocketReceiveResult(10, WebSocketMessageType.Text, true)


        //    //)};

        //    var mock = webSocketMock as Mock<IClientWebSocket>();

        //    var factoryMock = Mock.Of<IClientWebSocketFactory>(mf =>
        //        mf.New() == webSocketMock
        //    );

        var hc = new JoySoftware.HomeAssistant.Client.HassClient(wsFactory: factoryMock) { SocketTimeout = 20 };

        await hc.ConnectAsync(new Uri("ws://localhost:8123"), "TOKEN", false);
            await Assert.ThrowsAsync<OperationCanceledException>(async () => await hc.CloseAsync());
